# Rivet Package Updater Setup

B1.3.6 continues the platform-specific Rivet package feeds introduced by B1.3.2 **The Compatibility Update**, while retaining the old Windows installer feed as a migration bridge.
...
Rivet does not treat the public SemVer string as the whole release identity. Platform feeds publish both:
...
The editor compares release version first, then build number when the public version is equal. Therefore `1.3.6 Build 2` is newer than `1.3.6 Build 1`. Build 1+ requests updater feeds with no-cache headers and publishes each internal build as a distinct GitHub Release tag so GitHub's `releases/latest/download/...` route cannot remain pinned to an older same-version release.

## Package verification

The main application uses the Rust `tauri-plugin-updater` API for release discovery and downloading. The package signature is verified against Rivet's embedded public key before the downloaded bytes are handed to Rivet's own installer logic.
...
The styled Windows Update Service is copied to a temporary directory, backs up the currently installed runtime, replaces the files, rolls back on failure, and restarts Rivet.
...
For AppImage installs, Rivet copies its small Linux update helper out of the running AppImage, downloads and verifies the signed ZIP, and then exits. The helper validates the package, waits for the old process, keeps a rollback copy, replaces the AppImage, restores executable permissions, and relaunches Rivet.

### `.deb` installations

Debian/Ubuntu/Pop!_OS installs use the operating system's normal privilege boundary. Rivet downloads and verifies the signed package as the user, exits, then the Linux helper invokes `pkexec` so the desktop's Polkit agent can authorize `dpkg --install`. Rivet never displays a password box and never receives the user's password.
...
**Repository → Actions → Build and publish Rivet → Run workflow**
...
→ build Rivet NSIS installer
→ build/sign Windows Rivet ZIP
...
## Android editor preview (B1.3.6 Build 10)

The Build 10 Android target is deliberately an **editor-first preview**. It does not use the desktop Rivet updater or the `oxide-latest-*` feeds. Android preview builds are distributed as APK artifacts from GitHub Actions.
...
Build 10 uses a preview Android application identifier (`com.rivet.rde.preview`) and a debug-signed APK for development/testing. Stable production Android signing, package identity, update delivery, and the local Rust toolchain backend are intentionally deferred until the Android editor shell is proven reliable.

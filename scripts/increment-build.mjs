...
main = main.replace(/(?:OXIDE EDITOR|RIVET) · B\d+\.\d+\.\d+(?: · BUILD \d+)?/g, `RIVET · ${display} · BUILD ${packageJson.buildNumber}`);
main = main.replace(/(?:OXIDE|RIVET) B\d+\.\d+\.\d+(?: · BUILD \d+)?/g, `RIVET ${display} · BUILD ${packageJson.buildNumber}`);
...
console.log(`Rivet build number updated: ${current} -> ${packageJson.buildNumber}`);
